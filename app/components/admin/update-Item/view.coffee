...
firstitemsAttrTemplate = Handlebars.templates["admin/update-Item/templates/first-item-attr"]
seconditemsAttrTemplate = Handlebars.templates["admin/update-Item/templates/second-item-attr"]
thirditemsAttrTemplate = Handlebars.templates["admin/update-Item/templates/third-item-attr"]

class AdminUpdateItem
  constructor: ($) ->
  ...
  bindEvents: ->
    ...
    $(document).on "click", ".js-show-firstattr", @showFirstLevel
  ...
  organizeItem: ->
    ...
    {
      id: $(".js-item-update").data "id"
      type: $("span[name=type]").data "type"
      price: price
      itemCode: $("span[name=itemCode]").data "itemcode"
      itemName: $("span[name=itemName]").data "itemname"
      buyLimit: $("input[name=buyLimit]").val()
      mainImage: $("input[name=mainImage]").val()
      originPrice: $("span[name=originPrice]").data "originprice"
    }
  ...
  submitUpdate: (evt)->
    ...
    itemAttributeMap = that.organizeItemAttr()
    firstLevelLength = $(".firstLevel-block").length
    if firstAndSecondChoosed and !_.isEmpty(itemAttributeMap) or firstLevelLength is 0
      $.ajax
  ...
  showFirstLevel: ->
    ...
    typeValue = typeMatchMap[$(@).data("type")]
    if $(".single-attribute").length > 0
...
module.exports = AdminUpdateItem
